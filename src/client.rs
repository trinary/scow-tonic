...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
   ...
   let status_request = tonic::Request::new(
        StatusRequest { }
    );
    ...
    let get_request2 = tonic::Request::new(
        GetRequest {
            key: String::from("abc"),
        }
    );
    ...
    let vote_request = tonic::Request::new(RequestVoteRequest{ term: 10, candidate_id: 11, last_log_index: 12, last_log_term: 13 });
